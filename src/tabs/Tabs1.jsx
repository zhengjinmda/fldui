...
class Tabs extends Component {
  ...
  renderTabs() {
    ...
    React.Children.forEach(props.children, (tab, index) => {
      ...
      tabItemList.push(
        <div key={index} className={tabClassName} onClick={this.onTabClick.bind(this, index)}>
          {tab.props.header}
        </div>,
      );
      tabPanelList.push(
        <div key={index} className={panelClassName} style={{ width }}>
          {tab.props.children}
        </div>,
      );
    });
    ...
    return (
      <div className={className('cooTab')}>
        <div className={className('cooTab-header')}>
          <div className={className('cooTab-list cooClearfix')} ref={this.tabsRef}>
            {contents[0]}
          </div>
          <div className={className('cooTab-bar')} ref={this.barRef} />
        </div>
        <div className={className('cooTab-content')} ref={this.contentRef}>
          <div className={className('cooTab-wrapper cooClearfix')} style={wrapperStyle} ref={this.wrapperRef}>
            {contents[1]}
          </div>
        </div>
      </div>
